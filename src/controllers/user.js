...
exports.register = async (req, res) => {
    const { phone, email, address, name, fcm, password, confirm_password } = req.body;
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({ message: "user with this email or phone not exist" });
        }
...
exports.updateProfile = async (req, res) => {
    ...
    try {
        ...
        if (phone && phone !== user.phone && (await User.findOne({ where: { phone } }))) {
