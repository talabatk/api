...
const getVendorOrder = async (vendorId, id) => {
    ...
    const order = await Order.findByPk(id, {
        attributes: ["id", "status", "name", "phone", "total", "shipping", "address", "createdAt", "notes"],
        include: [
            {
                model: CartProduct,
                include: [
...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        for (const e of cart.cart_products) {
            ...
            if (!area) {
                return res.status(400).json({ message: "المطعم لا يدعم التوصيل لهذه المنطقه" });
            }

            const directionIndex = shippingDirections.findIndex((item) => item.vendor === e.product.vendorId);
            ...
            const vendorIndex = vendors.findIndex((item) => +item.vendorId === +e.product.user.vendor.id);
    ...
    try {
        ...
        orders = orders.map((order) => {
            ...
            if (+vendor.free_delivery_limit === 0) {
                shipping = areaCost.cost;
            } else if (+vendor.free_delivery_limit !== 0 && +vendor.free_delivery_limit > +total) {
                shipping = areaCost.cost;
            }
